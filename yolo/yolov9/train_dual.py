...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            fi = fi.item() if hasattr(fi, 'item') else float(fi)
            stop = stopper(epoch=epoch, fitness=fi)  # early stop check
            if fi > best_fitness:
                best_fitness = fi
                # Save best metrics to best.txt
                best_txt = save_dir / 'best.txt'
                mp, mr, map50, map_ = [float(x) for x in results[:4]]
                with open(best_txt, 'w') as f_best:
                    f_best.write(f'Best metrics (epoch {epoch})\n')
                    f_best.write(f'Fitness:    {fi:.6f}\n')
                    f_best.write(f'Precision:  {mp:.6f}\n')
                    f_best.write(f'Recall:     {mr:.6f}\n')
                    f_best.write(f'mAP@0.5:    {map50:.6f}\n')
                    f_best.write(f'mAP@0.5:95: {map_:.6f}\n')
                LOGGER.info(f'Best fitness {fi:.6f} at epoch {epoch}, saved to {best_txt}')
...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights', type=str, default='https://github.com/WongKinYiu/yolov9/releases/download/v0.1/yolov9-c.pt', help='initial weights path')
