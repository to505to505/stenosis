"""Evaluation script — compute mAP on the test (or valid) set."""
...
def evaluate_map(
    ...
    Args:
        all_detections: list of dicts with 'boxes' (N,4), 'scores' (N,)
        all_ground_truths: list of (G,4) GT boxes arrays

    Returns:
        AP at given IoU threshold
    """
    ...
    gt_matched = {i: np.zeros(gt.shape[0], dtype=bool) for i, gt in enumerate(all_ground_truths) if gt.shape[0] > 0}
