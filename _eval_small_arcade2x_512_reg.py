"""Run RF-DETR Small (arcade2x_512_reg) inference on both datasets."""
import json
import time
...
MODEL = {
    "name": "rfdetr_small_arcade2x_512_reg",
    "ckpt": ROOT / "rfdetr_runs" / "rfdetr_small_arcade2x_512_reg" / "checkpoint_best_total.pth",
    "out_dir": ROOT / "rfdetr_runs" / "rfdetr_small_arcade2x_512_reg",
    "resolution": 512,
}
DATASETS = [
    {
...
def eval_dataset(model, ds):
    img_dir, lbl_dir = ds["img_dir"], ds["lbl_dir"]
    all_images = sorted(img_dir.glob(ds["img_ext"]))
    print(f"\nDataset: {ds['name']}  ({len(all_images)} images)")
    ...
    out_path = MODEL["out_dir"] / f"eval_{ds['name']}.json"
    with open(out_path, "w") as f:
        json.dump(results, f, indent=2)
    print(f"Saved: {out_path}")
    preds_path = MODEL["out_dir"] / f"preds_{ds['name']}.json"
...
def main():
    # try checkpoint_best_total first, fall back to checkpoint_best_ema
    ckpt = MODEL["ckpt"]
    if not ckpt.exists():
        ckpt = MODEL["out_dir"] / "checkpoint_best_ema.pth"
        print(f"checkpoint_best_total.pth not found, using {ckpt.name}")
    ...
    all_results = {}
    for ds in DATASETS:
        all_results[ds["name"]] = eval_dataset(model, ds)
    ...
    for ds_name, r in all_results.items():
        ...
        for iou in [0.25, 0.3, 0.5]:
            print(f"  IoU≥{iou}: R={r[f'recall@IoU{iou}_conf{CONF_VIS}']:.4f}  P={r[f'precision@IoU{iou}_conf{CONF_VIS}']:.4f}  F1={r[f'f1@IoU{iou}_conf{CONF_VIS}']:.4f}")
