"""Training loop for Stenosis-DetNet.

SGD per the paper (lr=0.002, momentum=0.9) — we keep psstt's effective
recipe (warmup + cosine + grad accumulation) on top of SGD, which is the
proven shape for this dataset. Reuses :mod:`rfdetr_video.ema`,
:mod:`rfdetr_video.schedule`, and :mod:`rfdetr_video.selection` so model
selection / early stopping behave identically to the other trainers we
compare against.
"""
...
def write_best_txt(run_dir: Path, best_metrics: dict, best_epoch: int, cfg: Config):
    with open(run_dir / "best.txt", "w") as f:
        f.write("Stenosis-DetNet Best Results\n")
        f.write("=" * 40 + "\n\n")
...
def train(cfg: Config) -> Path:
    ...
    print(f"[DetNet] supervise_all_frames={cfg.supervise_all_frames}  "
          f"sca_enabled_eval={cfg.sca_enabled_eval}")
    ...
    for epoch in range(cfg.epochs):
        ...
        if (epoch + 1) % cfg.eval_interval == 0:
            metrics = evaluate(model, val_loader, cfg, device, apply_sca_pp=False)
            if ema is not None:
                with ema.applied_to(model):
                    ema_metrics = evaluate(model, val_loader, cfg, device, apply_sca_pp=False)
            else:
                ema_metrics = metrics
            # Optionally also report SCA-filtered metrics (eval-time only —
            # they don't affect selection so we don't perturb the existing
            # selection signal psstt was tuned on).
            sca_metrics = {}
            if cfg.sca_enabled_eval and ema is not None:
                with ema.applied_to(model):
                    sca_metrics = evaluate(
                        model, val_loader, cfg, device, apply_sca_pp=True,
                    )
            elif cfg.sca_enabled_eval:
                sca_metrics = evaluate(
                    model, val_loader, cfg, device, apply_sca_pp=True,
                )
            ...
            record = {
                ...
                **{f"sca/{k}": v for k, v in sca_metrics.items()},
                "sel": sel,
            ...
            sca_msg = ""
            if sca_metrics:
                sca_msg = (f" | sca mAP30={sca_metrics['AP@0.3']:.4f} "
                           f"F1={sca_metrics['F1']:.4f}")
            print(
                f"  val — mAP30={metrics['AP@0.3']:.4f} "
                f"| ema mAP30={ema_metrics['AP@0.3']:.4f} "
                f"ema mAP50={ema_metrics['AP@0.5']:.4f} "
                f"ema F1={ema_metrics['F1']:.4f}{sca_msg} "
            ...
            if cfg.wandb_enabled:
                ...
                for k, v in sca_metrics.items():
                    log_dict[f"val_sca/{k}"] = v
                wandb.log(log_dict, step=global_step)

            if sel_smoothed > best_sel:
                best_sel = sel_smoothed
                best_metrics = {
                    **ema_metrics, "sel_smoothed": sel_smoothed,
                    **{f"sca/{k}": v for k, v in sca_metrics.items()},
                }
                best_epoch = epoch + 1
...
def parse_args():
    p = argparse.ArgumentParser(description="Train Stenosis-DetNet on dataset2_split")
    ...
    p.add_argument("--no-sca-eval", action="store_true",
                   help="Disable SCA-filtered metrics during eval (faster).")
    p.add_argument("--centre-only", action="store_true",
                   help="Train only the centre frame (memory saver; not paper-faithful).")
    return p.parse_args()
