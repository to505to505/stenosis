"""Temporal sequence dataset for Stenosis-DetNet.
...
Normalization on 0-1 scale with mean=0.394, std=0.181.
"""
...
def build_sequence_index(image_dir: Path) -> List[Tuple[str, int, List[Path]]]:
    ...
    Returns list of (patient_id, seq_id, sorted_image_paths).
    """
...
def load_yolo_labels(label_path: Path, img_w: int, img_h: int) -> np.ndarray:
    """Load YOLO-format labels and convert to absolute x1y1x2y2.

    Returns array of shape (N, 5): [class_id, x1, y1, x2, y2].
    Empty files return shape (0, 5).
    """
...
def build_train_augmentation(img_h: int, img_w: int) -> ReplayCompose:
    """Build augmentation pipeline: translation, flipping, zoom.
    ...
    return ReplayCompose(
        [
            ...
            A.Affine(
                ...
                translate_percent=(-0.08, 0.08),   # translation
...
class StenosisDetNetDataset(Dataset):
    """Dataset yielding windows of T consecutive grayscale frames + labels.

    Includes an augmented copy of every training window to double
    ...
    def __init__(self, split: str, cfg: Config):
        ...
        # Double training set: first half = original, second = augmented
    ...
    def __getitem__(self, idx: int):
        # For doubled dataset: first half = original, second half = augmented
        use_aug = False
        real_idx = idx
        if self._double:
            n = len(self.windows)
        ...
        for frame_i, img_path in enumerate(paths):
            ...
            # Apply augmentations (same transform across all frames)
...
def collate_fn(batch):
    """Collate list of (images, targets) into batched tensors.

    Returns:
        images: Tensor of shape (B, T, 1, H, W)
        targets: list of length B, each a list of T dicts
    """
...
def get_dataloader(split: str, cfg: Config, shuffle: bool = False) -> DataLoader:
    ds = StenosisDetNetDataset(split, cfg)
    return DataLoader(
        ds,
