"""Quick smoke test: verify tensor shapes through the full Stenosis-DetNet pipeline."""
...
def main():
    ...
    print(f"  Got {len(results)} frame results (expected {B * T})")
    for r in results[:3]:
        n_dets = r['boxes'].shape[0]
        score_min = r['scores'].min().item() if n_dets > 0 else 0
        score_max = r['scores'].max().item() if n_dets > 0 else 0
        print(f"    batch={r['batch_idx']} frame={r['frame_idx']} "
              f"dets={n_dets} "
              f"score_range=[{score_min:.3f}, {score_max:.3f}]")
