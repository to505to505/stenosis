...
def _to_rfdetr_cfg(cfg: Config) -> RFVideoConfig:
    ...
    # Disable every optional branch that VideoStenosisDataset / loader sees.
...
class PSSTTVideoDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        for t_dict in targets:
            ...
            # Drop boxes with non-positive area (clipping could produce them).
...
def get_dataloader(split: str, cfg: Config, shuffle: bool) -> DataLoader:
    ds = PSSTTVideoDataset(split, cfg)
    return DataLoader(
        ds,
