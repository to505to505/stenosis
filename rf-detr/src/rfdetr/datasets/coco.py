...
def _build_train_resize_config(
    ...
    *,
    ...
    """
    # interpolation=4 is cv2.INTER_LANCZOS4 (best for upscaling fine vessel details);
    # area_for_downscale="image" automatically switches to cv2.INTER_AREA when
    # the image is being downscaled (avoids moiré / aliasing on thin vessel lines).
    _RESIZE_INTERP = {"interpolation": 4, "area_for_downscale": "image"}

    if square:
        option_a: Dict[str, Any] = {
            "OneOf": {
                "transforms": [
                    {"Resize": {"height": s, "width": s, **_RESIZE_INTERP}}
                    for s in scales
                ],
            }
        }
        option_b: Dict[str, Any] = {
            "Sequential": {
                "transforms": [
                    {"SmallestMaxSize": {"max_size": [400, 500, 600], **_RESIZE_INTERP}},
                    {
                        "OneOf": {
                            "transforms": [
                                {"RandomSizedCrop": {"min_max_height": [384, 600], "height": s, "width": s, **_RESIZE_INTERP}}
                                for s in scales
                            ],
                        }
                    },
        ...
        option_a = {
            "Sequential": {
                "transforms": [
                    {"SmallestMaxSize": {"max_size": size_param, **_RESIZE_INTERP}},
                    {"LongestMaxSize": {"max_size": cap, **_RESIZE_INTERP}},
                ]
            }
        }
        option_b = {
...
def make_coco_transforms(
    ...
    if image_set in ("val", "test"):
        resize_wrappers = AlbumentationsWrapper.from_config(
            [
                {"SmallestMaxSize": {"max_size": resolution, "interpolation": 4, "area_for_downscale": "image"}},
                {"LongestMaxSize": {"max_size": 1333, "interpolation": 4, "area_for_downscale": "image"}},
            ]
        )
        return Compose([*resize_wrappers, to_image, to_float, normalize])
    if image_set == "val_speed":
        resize_wrappers = AlbumentationsWrapper.from_config([{"Resize": {"height": resolution, "width": resolution, "interpolation": 4, "area_for_downscale": "image"}}])
        return Compose([*resize_wrappers, to_image, to_float, normalize])
