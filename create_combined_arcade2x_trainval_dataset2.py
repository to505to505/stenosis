#!/usr/bin/env python3
"""
Create a combined dataset (ARCADE train+val 2x oversampled + dataset2_split).

Same as create_combined_arcade_trainval_dataset2.py BUT each ARCADE image is
copied twice into the train split (with prefixes ``arcade_`` and
``arcade2_``) so that ARCADE samples appear in batches roughly twice as
often as dataset2 samples.

Splits:
    train:  arcade(train+val) + arcade(train+val)  +  dataset2_split/train
    valid:  dataset2_split/valid
    test:   dataset2_split/test

All classes are remapped to single class 0 (stenosis).
"""
...
COMBINED = BASE / "combined_arcade2x_trainval_dataset2"
...
def main():
    ...
    # === ARCADE train+val into train, copied TWICE with different prefixes ===
    print("=== Copying ARCADE train+val into train (2x oversampled) ===")
    total_arcade = 0
    for dup_prefix in ["arcade_", "arcade2_"]:
        for arc_split in ["train", "val"]:
            ...
            n = copy_with_labels(src_img, src_lbl, dst_img, dst_lbl,
                                 remap=ARCADE_REMAP, prefix=dup_prefix)
            print(f"  arcade/{arc_split} -> train (prefix={dup_prefix}): {n} images")
            total_arcade += n
    print(f"  Total ARCADE images in train (with duplication): {total_arcade}")

    # === dataset2_split train into train ===
    ...
    # === dataset2_split valid into valid ===
